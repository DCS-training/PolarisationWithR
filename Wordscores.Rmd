...
sample_docs <- data %>% sample_n(10)  # Randomly select 50 documents
...
dfm_ref <- dfm_remove(dfm_ref, pattern = stopwords("en"))  # Remove stopwords
```
...
virgin_docs <- data%>% 
  ...
  select(full_text)  # Select unscored docs

tokens_virgin <- tokens(virgin_docs$full_text, remove_punct = TRUE)
dfm_virgin <- dfm(tokens_virgin)
dfm_virgin <- dfm_remove(dfm_virgin, pattern = stopwords("en"))  # Remove stopwords

predicted_scores <- predict(ws_model, newdata = dfm_virgin)  # Predict scores
```
...
write_csv(data, "EU-China_with_scores.csv")  # Save updated dataset
```
...
### Repeat Process with Fixed Reference Documents

```{r fixed-reference}
data <- read_csv("EU-China_resolutions.csv")  # Reload original dataset

# Ensure reference documents exist in dataset based on "resolution code"
df_fixed_reference <- data %>%
  filter(resolution_code %in% c("P4_1998/0293", "P6_2009/0053"))
...
virgin_docs_fixed <- data %>% filter(!resolution_code %in% c("P4_1998/0293", "P6_2009/0053")) %>%
  select(full_text)
