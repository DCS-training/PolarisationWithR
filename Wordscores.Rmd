...
sample_docs <- data %>% sample_n(10)  # Randomly select 10 documents
...
```{r merge-data}
data <- data %>%
  left_join(df_reference %>% select(full_text, reference_score), by = "full_text")
```
...
```{r apply-model}
virgin_docs <- data %>% filter(is.na(reference_score)) %>% select(full_text)  # Select unscored docs
